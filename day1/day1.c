/* --- Day 1: Historian Hysteria ---
 ...
 to add up all of those distances. For example, if you pair up a 3 from the left
...
// Plan is to sort the two lists and start adding up the pairs

#include <stdio.h>
#include <stdlib.h>

// compare function for qsort to work
...
struct lists* readfile(char * array) {
  ...
  struct lists *p = malloc(sizeof(struct lists));
  ...
  for (int i = 0; i < 1000; i++) {
    fscanf(filep, "%5d", &p->leftnums[i]);
    fscanf(filep, "%5d", &p->rightnums[i]);
  }
...
int main() {
  struct lists *l = readfile("day1.txt");

  qsort(l->leftnums, 1000, sizeof(int), comp);
  qsort(l->rightnums, 1000, sizeof(int), comp);

  int sum = 0;
  for (int i = 0; i < 1000; i++) {
    sum += abs(l->leftnums[i] - l->rightnums[i]);
  }
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    for (int j = 0; j < 1000; j++) {
      if (l->leftnums[i] == l->rightnums[j]) {
        count++;
      } else if (count > 0) {
        break;
      }
    }
      score += l->leftnums[i] * count;
  }
